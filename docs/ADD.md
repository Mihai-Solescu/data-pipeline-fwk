...
To cleanly separate the complex logic of handling the parameter space from the workflow logic of the orchestrator, the framework uses a dedicated **`ParameterManager`** service. This class is the sole authority on generating, filtering, and projecting the parameter sets that define the scope of the experiment.

#### `ParameterManager` Core Responsibility

The `ParameterManager` is instantiated once at the beginning of a run with the `config.params` struct. Its constructor immediately performs the expensive, one-time computation of the definitive **`Approved_Runs`** list. This is achieved by:

1. Generating the full Cartesian product of the `config.params.grid`.
2. Merging each resulting combination with the `config.params.globals`.
3. Applying the `config.params.filter` function to discard invalid runs.

The resulting list of valid, complete parameter structs is stored immutably for the duration of the run.

#### Efficient Task Projection

The primary role of the `ParameterManager` during the orchestration phase is to provide the minimal sub-grid of tasks for any given stage. This is handled by the `getProjectedTasks` method.

* **Algorithm**: The method performs a **linear search** over the pre-computed `Approved_Runs` list. It iterates through this list exactly once. For each valid run, it performs the following steps:
    1. It creates a **canonical string key** representing the specific combination of values for the stage's `effective_param_names`. This key is generated by sorting the parameter names alphabetically and concatenating them with their corresponding values (e.g., `'embedding_dim_70_rank_50'`).
    2. This string is used as a key in a hash map (`containers.Map`) to track uniqueness.
    3. If the string key is new, it means this is the first time this specific combination of effective parameters has been seen. The method then creates the corresponding projected parameter struct and adds it to the list of tasks for the stage.

* **Complexity**: This is an **`O(M)`** algorithm, where `M` is the total number of approved runs. Its performance is directly proportional to the size of the experiment.

> **Note on Future Optimizations:** This linear search is a pragmatic and correct implementation for the initial version of the framework. For very large-scale experiments with millions of runs, a future release will include a more advanced data structure, such as a **Hierarchical Index Tree**, to achieve sub-linear (`O(log)`-like) performance. This will be a non-breaking change that can be implemented once the core framework is stable.

#### `ParameterManager` Public API

* **`obj = ParameterManager(config_params)`**: The constructor that takes the user's parameter configuration and pre-computes the `Approved_Runs` list.
* **`runs = getApprovedRuns()`**: Returns the complete, filtered list of all valid end-to-end parameter run structs.
* **`tasks = getProjectedTasks(effective_param_names)`**: Takes a cell array of parameter names and returns a cell array of unique sub-grid structs (the tasks) for a stage, calculated via the efficient projection algorithm. If the stage has no effective parameters, it returns an empty cell array and triggers a warning. If it gets the full parameter list, it returns the full `Approved_Runs` list.
