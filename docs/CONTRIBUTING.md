...
This class contains the test suite for the `ParameterManager` service.

* **`testGridGeneration`**: Verifies that the class correctly computes the full Cartesian product of a sample parameter grid.
* **`testGlobalParameterMerging`**: Confirms that the `globals` are correctly added to every single run generated from the grid.
* **`testFilterApplication`**: Verifies that a sample filter function is correctly applied and that only the runs returning `true` are included in the final `Approved_Runs` list.
* **`testTaskProjection`**: The most critical test. It provides a list of `Approved_Runs` and a set of `effective_param_names` and verifies that the `getProjectedTasks` method returns the correct, minimal set of unique task structs.
* **`testProjectionWithNoEffectiveParams`**: Verifies that projecting onto an empty set of effective parameters correctly returns a single, empty struct task.
* **`testEdgeCaseNoGridParams`**: Confirms that the manager works correctly when the `grid` is empty, resulting in a single run composed only of `globals`.
...
This taxonomy covers all logs and errors generated by the `pipeline:ParameterManager` service during the generation, filtering, and projection of the experimental parameter space.

| Identifier | Log Level | Description |
| :--- | :--- | :--- |
| `pipeline:ParameterManager:InvalidFilterFunction` | `FATAL` | The user-provided `config.params.filter` is not a valid function handle or has an incorrect signature. |
| `pipeline:ParameterManager:FilterFunctionError` | `FATAL` | An error occurred during the execution of the user-provided filter function on a parameter combination. |
| `pipeline:ParameterManager:NoApprovedRuns` | `WARN` | The combination of the parameter grid and the filter function resulted in zero valid runs for the experiment. |
| `pipeline:ParameterManager:GridGenerationComplete`| `INFO` | The full Cartesian product of the parameter grid has been generated, resulting in a specific number of potential runs before filtering. |
| `pipeline:ParameterManager:FilteringComplete` | `INFO` | The filter function has been successfully applied, resulting in a final, definitive list of approved runs for the experiment. |
| `pipeline:ParameterManager:ProjectionComplete` | `DEBUG` | The `getProjectedTasks` method successfully projected the approved runs onto a smaller set of effective parameters, resulting in a specific number of unique tasks for a stage. |
